...
class Path(Generic[R]):
    def __init__(self) -> None:
        self.rot_name_path: list[Optional[str]] = []
        self.node_path: list[AbstractCube[R]] = []
    ...
    def push(self, rotation_name: Optional[str], node: AbstractCube[R]) -> None:
        self.rot_name_path.append(rotation_name)
        self.node_path.append(node)

    def pop(self) -> tuple[Optional[str], AbstractCube[R]]:
        return (
    ...
    def contains_node(self, node: AbstractCube[R]) -> bool:
        return node in self.node_path


class IDAStar(Generic[R]):
    def __init__(self, h: Callable[[AbstractCube[R]], int]) -> None:
        self.h = h
        self.path = Path()
        self.bound = 0.

    def search(self, root: AbstractCube[R]) -> tuple[Path[R], Optional[int]]:
        root = root.copy()
    ...
    def dfs(self, node: AbstractCube[R], g: float, h: float) -> float:
        f = g + h
        if f > self.bound:
            return f
        if node.is_solved():
        ...
        min_t = float('inf')
        for name, succ, succ_h in successors(node, self.h):
...
if __name__ == '__main__':
    ...
    rot_name_seq, rot_seq = cube.shuffle(5)
